...
void test_ndef_writeMailtoNdef_given_kahlok0424_gmail_subtest_test_body_nfcTest(void)
{
  char email[30] = "kahlok0424@gmail.com";
  char subject[10] = "test";
  char body[40] = "testing to send email with content";
  uint8_t expected[90] = {0xD1,0x01,0x51,0x55,0x00,0x6d,0x61,0x69,0x6c,0x74,0x6f,
    0x3a,0x6b,0x61,0x68,0x6c,0x6f,0x6b,0x30,0x34,0x32,0x34,0x40,0x67,0x6d,0x61,0x69,0x6c,
    0x2e,0x63,0x6f,0x6d,0x3f,0x73,0x75,0x62,0x6a,0x65,0x63,0x74,0x3d,0x74,0x65,0x73,
    0x74,0x26,0x62,0x6f,0x64,0x79,0x3d,0x74,0x65,0x73,0x74,0x69,0x6E,0x67,0x20,0x74,0x6F,0x20,0x73,0x65,
    0x6E,0x64,0x20,0x65,0x6D,0x61,0x69,0x6C,0x20,0x77,0x69,0x74,0x68,0x20,0x63,0x6F,0x6E,0x74,0x65,0x6E,0x74};
  uint8_t CCexpect[4] = {0xe1,0x40,0x40,0x00};
  uint8_t TLVexpect[4] = {0x03,85};

  I2CWrite_ExpectWithArray(NFC_USERMEMORY, 0x00, CCexpect,4,4);
  I2CWrite_ExpectWithArray(NFC_USERMEMORY, 0x04, TLVexpect,2,2);
  I2CWrite_ExpectWithArray(NFC_USERMEMORY, 0x06, expected,60,60);
  NFC_Delay_Expect(100);
  I2CWrite_ExpectWithArray(NFC_USERMEMORY, 66, expected+60,25,25);

  writeMailto(email,subject,body);
}

void test_ndef_generateLaunchAppNdef_given_calculator_expect_launch_calculator(void)
{
  char appName[30] = "com.android.calculator2";
  uint8_t expected[42] = {0xD4,0x0f,0x17,0x61,0x6e,0x64,0x72,0x6f,0x69,0x64,0x2e,
    0x63,0x6f,0x6d,0x3a,0x70,0x6b,0x67,0x63,0x6f,0x6d,0x2e,0x61,0x6e,0x64,0x72,0x6f,0x69,0x64,
    0x2e,0x63,0x61,0x6c,0x63,0x75,0x6c,0x61,0x74,0x6f,0x72,0x32};
  uint8_t *result;
  uint8_t ndef[200];
  uint16_t size;
  size = generateLaunchAppNdef(appName,ndef);

  TEST_ASSERT_EQUAL_UINT16(41,size);
  TEST_ASSERT_EQUAL_UINT8_ARRAY(expected,ndef,40);
}
